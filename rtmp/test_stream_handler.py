...
class TestStreamHandler(unittest.TestCase):
    ...
    @patch('stream_handler.notify_backend')
    @patch('stream_handler.sys.argv', ['stream_handler.py', 'stop', 'stream-key-456', '{"duration": 3600}'])
    def test_main_with_metadata(self, mock_notify):
        ...
        self.assertEqual(cm.exception.code, 0)
        mock_notify.assert_called_once_with('stop', 'stream-key-456', {"duration": 3600})
    ...
    @patch('stream_handler.notify_backend')
    @patch('stream_handler.sys.argv', ['stream_handler.py', 'error', 'stream-key-789', 'invalid-json'])
